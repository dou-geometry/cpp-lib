    ...
    struct oricoord:coord<double> { // Only using double
        ...
        */
        oricoord& operator+=(const oricoord &r) {
            ...
            res[0]=(d[0]*d[0])+(r[0]*r[0])-2*d[0]*r[0]*std::cos();
            //unfinish
            //return *this;
        }
        ...
        friend oricoord operator-(oricoord lhs, const oricoord &r) { return lhs-=r; }
        template<typename O> requires std::convertible_to<O, T> friend oricoord operator*(oricoord lhs, const O &r) { return lhs*=(T)r; }
        template<typename O> requires std::convertible_to<O, T> friend oricoord operator*(const O &r, oricoord lhs) { return lhs*=(T)r; }
        template<typename O> requires std::convertible_to<O, T> friend oricoord operator/(oricoord lhs, const O &r) { return lhs/=(T)r; }
        template<typename O> requires std::convertible_to<O, T> friend oricoord operator/(const O &r, oricoord lhs) { return lhs/=(T)r; }
        // cross product overload
